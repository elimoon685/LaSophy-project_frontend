...
const NewBookComments = ({bookId, bookComments, setBookComments, userId}:NewBookCommentsPros) => {
  ...
  //this is place show all Comments no need self-referenced structure
  const renderCommentNote = (parent: GetCommentResponse) => {
        ...
        {commentIsFocused && 
          <div className="flex self-end mt-2">
            ...
            <button className={`font-bold cursor-pointer rounded-2xl px-2 py-1 ${!newComment.content.trim() && "bg-gray-200 text-gray-500"}`}
                    onClick={()=>submitReply(bookId, newComment.content)}
            >Comment</button>
          </div>
        }
      </div>
      <div className="flex max-w-[600px] flex-col">
        {bookComments.map(parent => renderCommentNote(parent))}
      </div>
    </div>
  )
}
export default NewBookComments